...
// Upload History table (for tracking YouTube uploads)
export const uploadHistory = sqliteTable(
  "upload_history",
  {
    ...
    categoryId: text("category_id").notNull().default("22"),
    categoryName: text("category_name"),
    videoUrl: text("video_url").notNull(),
    thumbnailUrl: text("thumbnail_url"),
    status: text("status").notNull().default("completed"), // completed, failed, processing
    fileSize: integer("file_size"), // in bytes
    duration: integer("duration"), // video duration in seconds (if available)
